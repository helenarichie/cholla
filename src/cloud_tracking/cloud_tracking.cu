...
void Cloud_Frame_Update(Real *dev_conserved, int nx, int ny, int nz, int n_ghost, int n_fields, Real dt, Real gamma, Real density_cloud_init, Real *integrand, Real *density_cloud_tot)
{
  ...
  hipLaunchKernelGGL(Cloud_Tracking_Kernel, launchParams.numBlocks, launchParams.threadsPerBlock, 0, 0, dev_conserved, nx, ny, nz, n_ghost, n_fields, dt, gamma, density_cloud_init, integrand_cloud.data(), density_cloud.data());
  CudaCheckError();
...
void Update_Grid_Velocities(Real *dev_conserved, int nx, int ny, int nz, int n_ghost, int n_fields, Real dt, Real gamma, Real velocity_cloud, Real density_cloud_tot)
{
  ...
  hipLaunchKernelGGL(Velocity_Update, launchParams.numBlocks, launchParams.threadsPerBlock, 0, 0, dev_conserved, nx, ny, nz, n_ghost, n_fields, dt, gamma, velocity_cloud, density_cloud_tot);
  CudaCheckError();
}
