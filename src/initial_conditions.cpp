      ...
      for (i=istart; i<iend; i++) {
        ...
        if (fabs(y_pos-0.5) < 0.25)
        {
          if (y_pos > 0.5)
          {
            ...
            C.momentum_y[id] = C.density[id] * A*sin(12*PI*x_pos) * exp( -0.5*pow(y_pos-0.75 - sqrt(-2.0*dy*dy*log(0.5)),2)/(dy*dy) ) ;
          }
          else
          {
        ...
        if (r < 0.25) // inside the cylinder
        {
          ...
          C.momentum_z[id] = sin(phi) * C.density[id] * A*sin(12*PI*x_pos) * exp( -0.5*pow(r-0.25 + sqrt(-2.0*dy*dy*log(0.5)),2)/(dy*dy) );
          mx = C.momentum_x[id];
          my = C.momentum_y[id];
          mz = C.momentum_z[id];
          C.Energy[id] = P/(gama-1.0) + 0.5*(mx*mx + my*my + mz*mz)/C.density[id];
        ...
        else // outside the cylinder
        {
          ...
          C.momentum_y[id] = cos(phi) * C.density[id] * A*sin(12*PI*x_pos) * (1.0 - exp( -0.5*pow(r-0.25 + sqrt(-2.0*dy*dy*log(0.5)),2)/(dy*dy) ));
          C.momentum_z[id] = sin(phi) * C.density[id] * A*sin(12*PI*x_pos) * (1.0 - exp( -0.5*pow(r-0.25 + sqrt(-2.0*dy*dy*log(0.5)),2)/(dy*dy) ));
          mx = C.momentum_x[id];
          my = C.momentum_y[id];
          mz = C.momentum_z[id];
          C.Energy[id] = P/(gama-1.0) + 0.5*(mx*mx + my*my + mz*mz)/C.density[id];
